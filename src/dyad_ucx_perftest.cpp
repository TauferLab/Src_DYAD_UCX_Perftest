...
int server_main (const Mode& mode, const std::string& tcp_addr, const size_t data_size)
{
    AbstractBackend* backend;
    switch (mode) {
        case Mode::TAG:
            backend = new TagBackend (AbstractBackend::SEND);
            break;
        default:
            throw std::runtime_error ("Invalid backend mode");
    }
    backend->init();
    Server* serv = new Server (data_size, tcp_addr, backend);
    try {
        serv->gen_data ();
        serv->start ();
        serv->run ();
                 ...
                 unsigned long int num_iters)
{
    ...
    switch (mode) {
        case Mode::TAG:
            backend = new TagBackend (AbstractBackend::RECV);
            break;
        default:
            throw std::runtime_error ("Invalid backend mode");
    }
    backend->init();
    Client* client = new Client (num_iters, data_size, tcp_addr, backend);
    try {
        client->start ();
        client->run ();
        client->shutdown ();
...
int main (int argc, char** argv)
{
    ...
    if (is_server) {
        return server_main (mode, tcp_addr, data_size);
    }
    return client_main (mode, tcp_addr, data_size, num_iters);
}